...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Spring%20Data%20JPA-3.x-success" alt="Spring Data JPA 3.x" />
...
3. Il réserve un créneau (avec prévention du double-booking via @Version).
...
# 3) Console H2 (dev)
http://localhost:8080/h2-console  (JDBC: jdbc:h2:mem:devdb)
...
# Notes :
# - Le seed de données est désactivé en profil test (pas de dépendance à H2/seed).
...
GET /api/slots?method=DELIVERY&day=2025-08-08
→ 200 OK
[
...
# Conseil (Spring AI)
GET /api/ai/advice?method=DELIVERY&day=2025-08-08
→ 200 OK
{ "advice": "..." }
# Cas d’erreur :
...
# Slots
curl "http://localhost:8080/api/slots?method=DELIVERY&day=2025-08-08"
...
# Conseil AI
curl "http://localhost:8080/api/ai/advice?method=DELIVERY&day=2025-08-08"
...
# Importez un fichier collection (si fourni) ou testez via cURL.
# Pensez à définir OPENAI_API_KEY dans les variables d’environnement si vous appelez /api/ai/advice.
